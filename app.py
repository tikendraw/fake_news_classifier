...
with st.spinner('Fill the boxes while we load the model... Predict button will be visible after model gets loaded'):
...
if submit:
	...
	with st.spinner('Predicting...'):
		...
		data = pd.DataFrame([[clean(input_title), clean(input_text)]], columns=['title', 'text'])
...
st.write('We achieved Great results on testing and validation results. Various Deep Learning Model. Here we are using **Model3: bigLSTM**')
st.image('./model_results1.png')

st.write('''**Note:** Incase our model mistakes to classify your article. It is probabily 
